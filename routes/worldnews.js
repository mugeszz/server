...
const worldNews = require("../model/world");
...
router.get("/getAllNews", async (req, res) => {
  try {
    const news = await worldNews.find();
...
router.get("/getOneNews/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const news = await worldNews.findById(id);
    res.status(200).send(news);
  } catch (error) {
    console.log(error);
    res.status(500).json({
...
router.put("/editNews", async (req, res) => {
  try {
    ...
    const news = await worldNews.findByIdAndUpdate(edit, id);
    res.status(200).send(news);
  } catch (error) {
    console.log(error);
    res.status(500).json({
